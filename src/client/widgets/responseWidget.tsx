...
export default function ResponseComponent({ requestIndex }: { requestIndex: number }) {
    ...
    return <div style={{
        ...
        {/* {secondary === 0 && <KeyValueRenderer data={res.response.headers} title={["Key", "Value"]} />}
        {secondary === 1 && <CookieRenderer data={res.response.cookie} />}
        {secondary === 2 && <ResponseViewer res={null} />}
        {secondary === 3 && <CookieRenderer data={res.response.cookie} />} */}
    </div>
}
